"""CLI entry point: train a PPO agent on the crypto environment.

Examples
--------
    python -m rl_trader.scripts.run_crypto_training
    python -m rl_trader.scripts.run_crypto_training --timesteps 50000 --seed 7
...
from ..config.training_config import crypto_config
from ..data.data_loader import load_ohlcv_csv
from ..training.train_crypto import train_crypto


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a PPO agent on crypto.")
...
def main() -> None:
    ...
    train_crypto(config=config, df=df)
