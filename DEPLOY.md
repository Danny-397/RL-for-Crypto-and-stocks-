...
The repo includes a [`render.yaml`](render.yaml) Blueprint, so you don't configure anything by hand.

1. Push the repo to GitHub (already done).
2. Go to **[dashboard.render.com](https://dashboard.render.com)** → **New +** → **Blueprint**.
3. Connect the GitHub repo `RL-for-Crypto-and-stocks-`. Render reads `render.yaml` and shows a service named **`rl-trader-api`** (Free plan).
4. Click **Apply**. Wait ~2–4 min for the first build.
5. You'll get a URL like `https://rl-trader-api.onrender.com`. Test it:
   - `https://rl-trader-api.onrender.com/health` → `{"status":"ok","policies":["crypto","stock"]}`
